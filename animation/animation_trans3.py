'''animation_trans3.py   将PNG格式图片单一背景颜色转换为透明   2022.11.18
在animation_trans2.py增加绘画窗口坐标线的函数coordinate_1(),这是用convert_alpha()
方法来修改Surface对象的像素格式为RGBA格式,含alpha通道A,当A=0时透明效果.
函数coordinate()与函数coordinate_1()效果相同,只是实现算法不同.
见本程序注释后带 ** 的语句,选其一.
...
#自定义绘画窗口坐标线的函数coordinate_1(),3个参数
def coordinate_1(win, color, interval):
    # win--拟绘坐标窗口Surface对象
    # color--元组或列表变量,坐标线条颜色    interval--整数变量,坐标线条间距
    surface = win.convert_alpha() # 修改图像win的像素格式,含alpha通道,赋值变量
    surface.fill((0,0,0,0))  # 将这个Surface对象底色充填为黑色(0,0,0,0),透明
    width = surface.get_width() # 获取Surface对象宽
    height = surface.get_height() # 获取Surface对象高
    for n in range(0,width+1,interval) :
        pygame.draw.line(surface,color,(n,0),(n,height),1) # 画坐标竖(y)线
    for n in range(0,height+1,interval) :
        pygame.draw.line(surface,color,(0,n),(width,n),1) # 画坐标横(x)线

    return surface # 返回绘画坐标线的的Surface对象

#自定义绘画窗口坐标线的函数coordinate(),4个参数
def coordinate(width, height, color, interval):
    # width--整数变量,拟绘坐标窗口宽   height--整数变量,拟绘坐标窗口高
    # color--元组或列表变量,坐标线条颜色    interval--整数变量,坐标线条间距
    surface = pygame.Surface((width,height)) # 创建一个width*height用于绘坐标线的Surface对象
    surface.fill((0,0,0))  # 将这个Surface对象底色充填为黑色(0,0,0)
...
# 自创建将PNG格式图片单一背景颜色转换为透明的函数,3个参数
...
if __name__ == "__main__":   # execute only if run as a script
    ...
    pygame.display.set_caption('Animation_Transparent  example 3 ') # 设置游戏窗口的标题

    # 调绘画窗口坐标线的函数coordinate(),以(166,200,255)坐标线颜色,间距=50
    # 生成坐标实例coordinate (Surface对象)
    #coordinate = coordinate(WIDTH, HEIGHT, (166,200,255), 50) # **
    coordinate = coordinate_1(screen, (166,200,255), 50) # **

    # 调用自创函数转换文件,r2.png文件背景色白色(255,255,255)转换透明
    #transparent('r.png','r2.png',(255,255,255))
    transparent('j1.png','j10.png',(255,255,255))
    # 从磁盘文件加载图片,转换成Surface对象
    RC = pygame.image.load('RC.png').convert_alpha()
    #r = pygame.image.load('r.png').convert_alpha()
    r = pygame.image.load('j1.png').convert_alpha()
    #r.set_colorkey((255,255,255)) # Surface对象r背景色白色(255,255,255)透明
    #r2 = pygame.image.load('r2.png').convert_alpha()
    r2 = pygame.image.load('j10.png').convert_alpha()
    ...
    person_r = Animation(r, 1, 8, 160, position_r)
    #person_r2 = Animation(r2, 1, 7, 160, position_r2)
    person_r2 = Animation(r2, 1, 8, 80, position_r2)
    ...
    while True: # 游戏主循环
        ...
        #screen.blit(coordinate,(0,0)) # 坐标线在窗口底层
        ...
        group.draw(screen)
        screen.blit(coordinate,(0,0)) # 坐标线在窗口最上层,与画在底层的语句选其一
