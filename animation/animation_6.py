'''animation_6.py   动画示例 6   对animation_5.py修改   2022.11.5
在animation_5.py基础上对Animation()及update()方法增加少量命令,就让人物走动起来.
...
class Animation(pygame.sprite.Sprite):
    ...
    # position- 绘制子图在窗口初始坐标位值,默认值x=0 y=0  speed- 移动速度 默认值=1
    def __init__(self, fileName, rows, columns, rate, position=[0,0],speed=1): # **
    ...
    def update(self, time):
        ...
        self.rect.x += self.speed # 帧的Rect对象x坐标增加speed (效果为向右移动) **
        if self.rect.x > WIDTH: # 如果走出窗口右边界 **
            self.rect.x = 0 - self.frameRect.width/2 # 重新从窗口左边出现 **
