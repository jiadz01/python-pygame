'''animation_trans.py   PNG格式图片动画测试和背景颜色转透明工具 2022.12.6
...
# 根据拟转换的图片和测试进行设置
filename1 = 'superman0.png' # 拟转换的图片文件名和扩展名
color = (255,255,255) # 拟转换的原图背景色RGBA颜色值
rows = 1 # 拟转换的原图的子(帧)图行数
columns = 8 # 拟转换的原图的子(帧)图列数
rate1 = 160 # filename1文件帧图播放间隔毫秒数
filename2 = 'superman00.png' # filename1转换后文件名和扩展名
rate2 = 80 # filename2文件帧图播放间隔毫秒数
...
if __name__ == "__main__":   # execute only if run as a script
    ...
    pygame.display.set_caption('Animation_Transparent tool ') # 设置游戏窗口的标题

    # 调绘画窗口坐标线的函数coordinate(),以(166,200,255)坐标线颜色,间距=50
    # 生成坐标实例coordinate (Surface对象)
    coordinate = coordinate_1(screen, (166,200,255), 50)

    # 调用自创函数转换文件,filename1文件背景色color转换透明
    transparent(filename1,filename2,color)
    # 从磁盘文件加载图片,转换成Surface对象
    RC = pygame.image.load('RC.png').convert_alpha()
    r = pygame.image.load(filename1).convert_alpha()
    #r.set_colorkey(color) # Surface对象r背景色color透明
    r2 = pygame.image.load(filename2).convert_alpha()

    position = [250,120] # 设置person在窗口坐标,x=250  y=120
    position_r = [50,80] # 设置r在窗口坐标,x=50  y=80
    position_r2 = [400,80] # 设置r2在窗口坐标,x=400  y=80
    ...
    person_r = Animation(r, rows, columns, rate1, position_r)
    person_r2 = Animation(r2, rows, columns, rate2, position_r2)
