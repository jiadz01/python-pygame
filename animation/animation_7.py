'''animation_7.py   动画示例 7   对animation_6.py另一种写法   2022.11.6
在animation_5.py基础上对Animation()及update()方法增加少量命令,新定义帧移动方法
move(),让人物走动起来.具体新增或变化语句,详见下边语句后 # 注释带 ** 的语句.
...
b.以pygame的sprite模块中名为精灵Sprite类为父类(基类),创建一个子类Animation来实现编程,
  该子类还新加一个动画绘制方法(每执行一次绘一帧)update()和move()帧移动方法.
...
class Animation(pygame.sprite.Sprite):
    ...
    # position- 绘制子图在窗口初始坐标位值,默认值x=0 y=0  speed- 移动速度 默认值=[0,0]
    def __init__(self, fileName, rows, columns, rate, position=[0,0],speed=[0,0]): # **
        ...
        self.frameRect = self.masterRect.copy() # 将masterRect拷贝到frameRect供每帧矩形框用
        self.rect = self.masterRect.copy() # 将masterRect拷贝到rect供动画每帧矩形框用 **
        ...
        self.frameRect.height = self.masterRect.height // rows # 计算出每帧的高,商取整数 **
        ...
        # 初始化帧图片的Rect对象 self.rect 的各个属性值,当用group.update()更新帧,group.draw()
        # 绘制帧image时,系统将自动(默认)调用self.rect,取rect坐标的属性值为帧image在窗口的坐标位值
        self.rect.width = self.frameRect.width # rect属性width=帧宽 **
        self.rect.height = self.frameRect.height # rect属性height=帧高 **
        self.rect.center = position # rect中心坐标=position **
        self.speed = speed # rect移动速度 即动画袳动速度 **

    # 定义帧移动(x轴方向从左向右移动)方法,移动Rect对象 **
    def move(self): # **
        # 调用Rect方法move(x,y),移动Rect对象.  x,y是对应坐标轴移动偏移量 **
        self.rect = self.rect.move(self.speed) # 移动后的Rect对象赋予原来的变量 **
        if self.rect.left > WIDTH: # 如果rect属性left大于窗口宽,即已移出窗口右边 **
            self.rect.right = 0 # rect属性right=0 ,即重新从窗口左边移动入屏幕 **
    ...
    def update(self, time):
        ...
        if self.frame != self.old_frame: # 如果现帧序号与已播放帧号不同,则创建拟播帧image
            # 按公式计算现帧序号对应的帧的frameRect矩形方框(Rect对象)的属性x,y值
        ...
        if self.current_time > self.last_time + self.rate: # 如大于换帧间隔毫秒数
            ...
            self.move() # 调本类自定义帧移动(x轴方向从左向右移动)方法,移动帧 **
...
if __name__ == "__main__":   # execute only if run as a script
    ...
    pygame.display.set_caption('Animation example 7 ') # 设置游戏窗口的标题

    position1 = [160,220] # 设置person1在窗口坐标,x=160  y=220  **
    # 调用自创建子类Animation()创建实例 person1,参数:总图文件名  总图行数=1 总图列数=8
    # 不相同的子图播放间隔毫秒数=80   子图初始中心坐标位值x=160  y=220  袳动速度 x=3,y=0
    person1 = Animation('RC.png', 1, 8, 80, position1,[3,0]) # **

    position2 = [460,220] # 设置person2在窗口坐标,x=460  y=220 **
    # 调用自创建子类Animation()创建实例 person2,参数:总图文件名  总图行数=1 总图列数=8
    # 不相同的子图播放间隔毫秒数=80   子图初始中心坐标位值x=460  y=220  袳动速度 x=2,y=0
    person2 = Animation('RC.png', 1, 8, 160, position2,[2,0]) # **
