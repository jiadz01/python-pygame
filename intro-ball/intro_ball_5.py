'''intro_ball_5.py 弹跳的球5  对intro_ball_4.py扩展修改    2022.6.8
新增一个精灵(boss)在窗口中由用户按下4个方向(上下左右)键进行移动,检测它是否与
弹跳的球发生碰撞,如有碰撞则球消失,另一个新球再从顶部弹出.
pygame提供了多种碰撞的检测方法:包括矩形碰撞检测、圆形碰撞检测和使用mask的精准碰撞检测.
本例用Rect模块中的colliderect()方法来检测是否有重叠发生,
矩形(Rect)碰撞检测方法,是有一些误差和缺陷.
本例示范游戏编程最基本的入门结构,内容是一个彩色球在有背景的游戏窗口中自动弹跳,
和图像bass由用户控制它上下左右进行移动:
  1.移动鼠标在游戏窗口右上角处"X"关闭按钮,点击鼠标左键退出此游戏程序.
  2.按键盘q或Q键,退出此游戏程序.
  3.图片文件intro_ball.gif,singapore2.jpg和boss.png,音乐文件ballVoice.wav
...
pygame.display.set_caption('Intro ball 5')
...
while True:
    ...
    if newBallRect.colliderect(bossRect):
          # 当球碰到精灵后被吸入消失时音效声
