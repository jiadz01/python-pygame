'''intro_ball.py 弹跳的球  对intro_ball_9a.py修改成范例版本 2022.9.27
a.本例用Pygame中处理图形遮罩(位掩码)的模块mask,创建记录该角色Surface对象的透明点
和不透明点的 mask对象,用模块mask类的overlap(other, offset)方法,
实现精确到1个像素级别的的碰撞检测,原理是测试两个子画面之间的位掩码是否重叠.
详见下边语句后 # 注释带 ** 的4条语句.
b.本例是每帧画面全部重新画(刷新)方式,感觉速度和效果都好,程序更简洁.
c.增加记分功能,详见下边语句后 # 注释带 * 的语句.
...
和图像bass由用户控制它上下左右进行移动:
  ...
  4.用户按下4个方向(上下左右)键进行精灵boss移动,检测它是否与弹跳的球发生碰撞,
    如有碰撞则加计1分,球将消失,另一个相同新球再从顶部弹出.
  5.本例生成N个不同图形,不同大小和不同移动速度的弹跳球,
    如果两个球之间发生碰撞则改变它们弹跳方向.
对照这9个intro_ball_1...9a版本变化,可了解和比较pygame模块编写2D游戏最基本语句.
'''
# 导入所需的模块
...
def main():
    ...
    while True:
        ...
        for each in ball_list:
            ...
            if each.mask.overlap(bossMask,offset) != None: #未碰撞返回None **
                # 当球碰到精灵后被吸入消失时音效声
