'''intro_ball_3.py 弹跳的球3  对intro_ball_2.py扩展修改    2022.6.3
每次屏幕刷新把整个背景图重新绘制一遍太耗费计算机性能,本例用update()带参数方式
只对图片有改变的区域进行刷新,可以获得最高的刷新速率,让复杂的游戏都不会有卡顿现象.
...
pygame.display.set_caption('Intro ball 3') # 也可不用,使用默认名pygame window
...
# 用transform模块的scale()方法,图片大小缩放到60X60
