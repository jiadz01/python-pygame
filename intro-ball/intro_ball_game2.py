'''intro_ball_game2.py  弹跳的球游戏2   对intro_ball_game1.py修改 2022.10.19
本例修改和增加部份语句,让碰撞点显示图标稍长,更明显.
具体新增或变化语句,详见下边语句后 # 注释带 ** 的语句.
注意与intro_ball_game1.py程序语句对照,可进一步理解其变化.
...
pygame.display.set_caption('Bouncing ball game 2')
...
def main():
    ...
    redTime = greenTime = 0 # 初始化控制碰撞点绘画次数2个变量  **
    # 游戏主循环 game loop
    while True:
        ...
        for each in ball_list:
            ...
            if point != None: #未碰撞返回None
                ...
                if each.score == -1: # 如果球分值是减分
                    bossSpeed -= 1 # 减少boss的移动速度
                    redRect.center = coordinate # 设置红色闪光点rect的坐标位置
                    redTime = 4 # 碰撞点绘图红色闪电图4帧 **
                else: # 否则球分值是加分分
                    bossSpeed += 1 # 增加boss的移动速度
                    greenRect.center = coordinate # 设置绿色圆点rect的坐标位置
                    greenTime = 4 # 碰撞点绘图绿色圆圈图4帧 **
                # 当球碰到精灵后被吸入消失时音效声
        ...
        if redTime > 0: # 该值>0,则绘红色闪电 **
            screen.blit(red,redRect) # 在窗口绘制红色闪光点 **
            redTime -= 1 # **
        if greenTime > 0: # 该值>0,则绘绿色圆点 **
