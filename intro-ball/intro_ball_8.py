'''intro_ball_8.py 弹跳的球8  对intro_ball_7.py扩展修改    2022.7.28
本例是每帧画面仅刷新发生变化部分的区域,而不是整个屏幕,理论上效率更高,
但当多精灵移动交叉时可明显看到图片矩形区域特征(?)
...
pygame.display.set_caption('Intro ball 8')
...
class Ball(pygame.sprite.Sprite):
    ...
    def __init__(self, image, ballSize, position,speed ,voice):
        ...
        self.old_rect = self.rect # 保留球移动前的位置
...
def main():
    ...
    while True:
        ...
        if ballNumber > 0 and ballGenerateTime % 90 == 0:
              ...
              Ball(image, ballSize, position,speed, voice)
              # 调整控制变量值
