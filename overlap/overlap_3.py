''' overlap_3.py   mask的精准碰撞检测示范程序3 对overlap_2.py修改  2022.10.16
本例扩展了自创建类Role(),同时用它创建了5个不同形状的固定图形.
本例用Pygame中的 sprite 模块的 spritecollideany(sprite,group,collided=None )
和 collide_mask(sprite1, sprite2) 命令,实现精确到1个像素级别的的碰撞检测.
通过本例运行并与overlap_1.py,overlap_2.py程序语句对照,
可更好理解sprite模块精准碰撞检测方法,学习编程技术.
1.用鼠标移动蓝色小球去碰撞5个不同形状的固定图形,在窗口标题栏将显示碰撞参数值,
  窗口背色也会发生改变.
...
# 利用for循环调自创建类Role()创建生成5个拟静止不动图形实例
for n in range(5):
    if n >= 3:
        # 调自创建类Role()创建箭头图形实例image
        image = Role(sizes[n], n, positions[n], color1s[n],color2)
    else:
        # 调自创建类Role()创建非箭头外的其它图形实例image
        image = Role(sizes[n], n, positions[n], color1s[n])
...
while game_on:
    ...
    if point: # 如发生碰撞,获取第一个重叠交点
        ...
        pygame.display.set_caption('Pygame example: overlap_3.py      ( No collision )')
