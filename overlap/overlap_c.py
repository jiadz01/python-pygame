''' overlap_c.py   mask的精准碰撞检测示范程序c 对overlap.py修改  2022.10.16
用sprite模块的Group()类的方法来实现,详见下边语句后 # 注释带 ** 的语句.
用pygame.mask.from_threshold()创建某颜色color1的部份图形实现碰撞,此时颜色参数必须
是RGB255颜色模式值, 或加pygame.Color(color)处理解决,见下边语句后 # 注释带 *** 的语句.
通过本例运行并与overlap.py ,overlap_1.py,overlap_2.py,overlap_3.py程序语句对照,
...
def main():
    ...
    color1s = [(0,186,255),'red','green','red','red'] # 画图像的颜色值列表 ***
    ...
    while True:
        ...
        if point: # 如发生碰撞,获取第一个重叠交点
            ...
            pygame.display.set_caption('Pygame example: overlap_c.py      ( No collision )')
            screen.fill((16,116,26)) # 填充screen对象绿色背景 (效果清屏)

        # 用for ..in迭代列表,遍历整个group,**
        # 用Role()的draw在游戏窗上(screen) 绘制图像方法将图形实例绘制在screen上
        for each in group: # **
            each.move() # **
            #each.draw(screen) # 图形实例绘制在screen上(与下条语句等效) **

        group.draw(screen) # 利用Group的draw()方法将整个组的实例绘制在screen上 **
