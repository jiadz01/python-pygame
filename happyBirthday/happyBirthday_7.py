'''happyBirthday_7.py         对happyBirthday_6.py修改   2022.9.16
实现精确到1个像素级别的的碰撞检测原则是测试两个子画面之间的位掩码是否重叠.
本例用Pygame中处理图形遮罩(位掩码)的模块mask,创建记录该角色Surface对象的透明点
和不透明点的 mask,用Pygame中的精灵模块Sprite的 pygame.sprite.collide_mask()
2条语句就实现精确的碰撞检测,详见下边语句后 # 注释带 ** 的语句.
