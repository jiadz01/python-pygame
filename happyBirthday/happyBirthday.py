'''happyBirthday.py    对happyBirthday_8.py修改   2022.9.17
在主函数main()中增加一个总循环,增加game_1()函数参数传递方式,
将生成immortal实例的语句块,全部移至main()中总循环中的调用game_2()函数前,
这样当进入游戏第二部份祝福时,可按键盘空格键重新开始游戏.
详见下边语句后#注释带 ** 的语句.
本例用pygame编写的生日祝福,游戏编程最基本的入门结构:
  ...
  5.当进入游戏第二部份祝福时可按键盘空格键重新开始游戏.
认真对照这几个happyBirthday_1.....happyBirthday_8逐步改写和扩展版本,
可学习掌握用python的pygame模块,编写游戏等基本框架结构和编程技术.
'''
# 导入所需的模块
...
# 祝福词文本内容
text = "Hi Jia YiChen.    Happy birthday!    Grandpa and grandma love you!"
...
voice0.set_volume(1) # voice0音量大小参数n范围0.0---1.0
...
gift_voice = [voice1,voice2,voice3] # 设置3个gift不同图片音效Sound对象列表
...
class Role(pygame.sprite.Sprite):
    ...
    def __init__(self, image, roleSize, position,speed=None ,voice=None):
        ...
        self.mask = pygame.mask.from_surface(self.role)
    ...
    def immor_move(self):
        ...
        # 每碰一次窗口边则会按一定比例由小变大,直到原图实际大小
          if self.size[0] <= 290:
                # 按一定宽高比例增大immortal宽高值
                self.size[0] += 30 # 每次图宽增大30
                self.size[1] += 1.6310344827586 *30 # 每次图高增大的数量
                # 角色图片对象缩放到self.size要求大小
                self.role = pygame.transform.scale(self.image, self.size)
                if self.rect.left < 0:
                  ...
                  self.rect = self.role.get_rect()
                  # 设置碰窗口右边增大后初始位置
                  self.rect.right, self.rect.top = WIDTH, self.position[1]

# 自创建游戏第一部份循环 1 函数
# 注意giftNumber,giftGenerateTime这2个形参只在本函数内有效
# 不会改变(影响)函数体外的同名变量值
def game_1 (giftNumber,giftGenerateTime): # 形参传入初始值 **
  # 设置全局变量,以便在调用函数中使用相关变量数据
  global text1,text1Rect,choice,score # **
  ...
  while game_on:
      ...
      for each in gift_list:
          ...
          # 2.检测并处理2个Sprite对象gift,boss是否精确碰撞
          if pygame.sprite.collide_mask(each,boss):
...
def game_2 ():
  # 设置全局变量,以便在调用函数中使用相关变量数据
  global text2Rect
  game_on = True # 只有当 game_on = False才会退出循环2
  while game_on:
...
def main():
  # 设置全局变量,以便在调用函数中使用相关变量数据
  global immortal,boss,text1,text2,text1Rect,score,text2Rect
  # 生成boss实例
  bossPosition = [WIDTH/2,HEIGHT/2] # 设置boss初始位置在窗口中央
