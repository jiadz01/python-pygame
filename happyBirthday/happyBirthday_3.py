'''happyBirthday_3.py         对happyBirthday_2.py扩展修改   2022.7.18
自创建礼物类Gift(),可生成许多不同图形,不同大小和不同下落速度的礼品实例.
...
class Gift(pygame.sprite.Sprite):
    ...
    def __init__(self, image, giftSize, position,speed ,voice):
        ...
        self.size = giftSize # 设置礼物的宽和高,列表变量
        self.old_rect = self.rect # 初始化保留礼物移动前的位置
...
while game_on:
    ...
    for each in gift_list:
        ...
        # 3.对列表遍历的(每个)gift进行清屏重画和将变化的rect添加到参数列表
         # 原gift位置区域填充对应区域背景(清屏)
        screen.blit(bg1,each.old_rect,each.old_rect)
    ...
    rectangle_list.append(text1Rect)
